...
from utils import mask_coordinates_and_log_risk
...
async def predict(
    ...
    latitude: Optional[float] = Query(None, description="Tarla enlemi (opsiyonel)"),
    longitude: Optional[float] = Query(None, description="Tarla boylamı (opsiyonel)"),
    province: Optional[str] = Query("Bilinmiyor", description="İl (opsiyonel)"),
    district: Optional[str] = Query("Bilinmiyor", description="İlçe (opsiyonel)"),
    crop_name: Optional[str] = Query("Genel", description="Ürün türü (opsiyonel)"),
    ...
    try:
        ...
        res = model_loader.predict(image_bytes, confidence_threshold=threshold)

        # Eğer koordinat bilgisi sağlandıysa maskele ve risk loguna kaydet
        if latitude is not None and longitude is not None:
            try:
                risk_log = mask_coordinates_and_log_risk(
                    db=db,
                    latitude=latitude,
                    longitude=longitude,
                    province=province or "Bilinmiyor",
                    district=district or "Bilinmiyor",
                    crop_name=crop_name or "Genel",
                    detected_disease=res.get("prediction", "Bilinmeyen"),
                    confidence=res.get("confidence", 0.0)
                )
                res["risk_log"] = {
                    "id": str(risk_log.id),
                    "grid_code": risk_log.grid_code,
                    "risk_level": risk_log.risk_level,
                    "province": risk_log.province,
                    "district": risk_log.district
                }
            except Exception as log_err:
                print(f"WARNING: Risk kaydı oluşturulamadı: {log_err}")
...
@app.get(
    "/risk/regional-summary",
    summary="Bölgedeki hastalık risk seviyesini hesaplar ve özet rapor döner",
    description="Son N gündeki anonymous_risk_logs verilerine göre il, ilçe ve ürün bazlı vaka sayılarını, risk seviyesini (Düşük, Orta, Yüksek, Kritik) ve en sık görülen 3 hastalığı hesaplayıp döndürür."
)
def get_regional_risk_summary_endpoint(
    province: Optional[str] = Query(None, description="İl adı filtresi (opsiyonel)"),
    district: Optional[str] = Query(None, description="İlçe adı filtresi (opsiyonel)"),
    crop_name: Optional[str] = Query(None, description="Ürün türü filtresi (opsiyonel)"),
    days: int = Query(14, ge=1, le=365, description="Analiz edilecek geçmiş gün sayısı (varsayılan: 14)"),
    db: Session = Depends(get_db)
):
    try:
        return get_regional_risk_summary(
            db=db,
            province=province,
            district=district,
            crop_name=crop_name,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Bölgesel risk özeti hesaplanamadı: {e}"
        ) from e
